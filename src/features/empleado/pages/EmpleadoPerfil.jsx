...
import {
  User, Briefcase, Lock, Eye, EyeOff,
  Save, Camera, CheckCircle, Upload,
  AlertCircle, Loader
} from 'lucide-react'
...
export default function EmpleadoPerfil() {
  ...
  useEffect(() => {
    usuariosAPI.getMe()
      .then(res => {
        ...
        updateUser({ foto_url: d.foto_url || null })
      })
  ...
  const subirFoto = async () => {
    ...
    try {
      ...
      const res = await usuariosAPI.updateFoto(fd)
      const nuevaFoto = res.data?.foto_url || res.data?.url || fotoURL
      setFotoURL(nuevaFoto)
      setFotoFile(null)
      setFotoOk(true)
      updateUser({ foto_url: nuevaFoto })
      setTimeout(() => setFotoOk(false), 2500)
    } catch (err) {
      setFotoError(getErrorMessage(err, 'Error al subir la foto.'))
    } finally {
  ...
  const guardarPerfil = async () => {
    ...
    setErrorGuardar('')
    try {
      const res = await usuariosAPI.updateMe(form)
      updateUser({ nombre: res.data?.nombre ?? form.nombre })
      setGuardado(true)
      setTimeout(() => setGuardado(false), 2500)
    } catch (err) {
      setErrorGuardar(getErrorMessage(err, 'Error al guardar.'))
    } finally {
      setGuardando(false)
    }
  }
  ...
  const actualizarPass = async () => {
    ...
    try {
      ...
      setErrPass(getErrorMessage(e, 'Error al actualizar contraseña'))
    } finally {
      setCambiando(false)
    }
  }
